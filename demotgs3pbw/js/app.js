...
const app = new Vue({
    ...
    data() {
        return {
            ...
            currentLocalTime: '', loadingText: '',
            authMode: 'login', // State pengontrol layar login / register
            userNIM: '053977558', // Default NIM tugas
            loginForm: { email: 'admin@ut.ac.id', password: 'admin123' },
            registerForm: { nama: '', nim: '', email: '', password: '' },
            // Database lokal menampung akun bawaan awal tugas
            usersDatabase: [
                { email: 'admin@ut.ac.id', password: 'admin123', nama: 'Administrator', nim: '053977558' }
            ],
    ...
    computed: {
        ...
        // FIX: LOGIKA OTOMATISASI SALAM BERDASARKAN WAKTU JAM REAL-TIME
        greetingText() {
            const hours = new Date().getHours(); // Mengambil angka jam saja (0 - 23)
            if (hours >= 5 && hours < 11) {
                return 'Selamat pagi';
    ...
    methods: {
        ...
        handleLogin() {
            // Cek kecocokan data login dengan database user array reaktif
            const matchedUser = this.usersDatabase.find(
                u => u.email.toLowerCase() === this.loginForm.email.toLowerCase() && u.password === this.loginForm.password
            );

            if (matchedUser) {
                this.loadingText = 'Menautkan Kredensial Ke Server UT...';
                this.isLoading = true;
                this.userNIM = matchedUser.nim; // Set NIM dinamis sesuai yang login
                
                setTimeout(() => {
                    this.isLoading = false;
                    this.isLoggedIn = true;
                    this.showToastNotification(`Selamat Datang, ${matchedUser.nama}!`);
                }, 1000); 
            } else {
                this.triggerSimpleAlert('Login Gagal', 'Email atau password salah! Gunakan akun default atau buat baru.');
            }
        },
        handleRegister() {
            // Cek apakah email sudah terdaftar sebelumnya
            const isExist = this.usersDatabase.some(u => u.email.toLowerCase() === this.registerForm.email.toLowerCase());
            if (isExist) {
                this.triggerSimpleAlert('Registrasi Gagal', 'Email ini sudah terdaftar di sistem.');
                return;
            }

            this.loadingText = 'Mendaftarkan Akun Baru Ke Sistem...';
            this.isLoading = true;

            setTimeout(() => {
                // Simpan data pendaftaran baru ke dalam database lokal reaktif
                this.usersDatabase.push({
                    email: this.registerForm.email,
                    password: this.registerForm.password,
                    nama: this.registerForm.nama,
                    nim: this.registerForm.nim
                });

                this.isLoading = false;
                this.authMode = 'login'; // Balikkan ke form login
                
                // Set data input login otomatis terisi dengan akun baru biar instan
                this.loginForm.email = this.registerForm.email;
                this.loginForm.password = this.registerForm.password;

                // Reset form registrasi
                this.registerForm = { nama: '', nim: '', email: '', password: '' };
                this.showToastNotification('Akun Baru Berhasil Dibuat! Silakan Login.');
            }, 1000);
        },
        handleLogout() {
            this.loadingText = 'Membersihkan Sesi Logistik...';
            this.isLoading = true;
            ...
            setTimeout(() => {
                ...
                this.showToastNotification('Sesi Berakhir. Anda Berhasil Keluar.');
            }, 1000);
        },
